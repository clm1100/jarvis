...
$(document).ready(function() {
  ...
  if(table.length > 0) {
    ...
    $('#is_publish_filter, #start_date_filter, #end_date_filter').change(function() {
      datatables.draw();
    });
  }
  ...
  if(contentTextarea.length > 0) {
    ueTopics = UE.getEditor('content-textarea');
    ueTopics.ready(function() {
      $('#myTabContent .topics_add').click(function() {
        var link = '<link data="topics_link" href="http://dev.images.moretao.com/themes/topics/1/theme3.css" rel="stylesheet">';
        var content = ueTopics.getContent();
        if(content.indexOf('topics_link') === -1) {
          ueTopics.setContent(link + content);
        }
        ...
        var str = $(this).html();
        ueTopics.setContent(str, true);
      });
    });
  }

  // 原创预览;
  $('#topics_preview').click(function() {
    var content = '<link href="http://statics.moretao.com/stylesheets/application.css?version=201510211710" rel="stylesheet">' + ueTopics.getContent();
...
// 原创排序
function sort() { window.location.href = '/topics/sortable'; }

// 创建
function create() { window.location.href = '/topics/new'; }
...
function searchCommodities() {
  ...
  var api = '/api/commodities/search/' + keyword + '/0';
  $.get(api, function(data) {
    var html = tpl(data);
    $('#commodity-result-list').find('ul').html(html);
  });
  return true;
}
...
function addCommodities() {
  var ids = $('#commodity-result-list').find('input[name=commodities]:checked').map(function(i, ele) {
    return $(ele).parent();
  }).get();

  $('#commodities-list-group').append(ids);
...
function removeComments() {
  ...
  var api = '/topics/' + id + '/comments/' + cid;
